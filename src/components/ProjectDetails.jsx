...
const ProjectDetails = () => {
  ...
  if (!data?.listOfProjects) return <div className="no-data">No data available.</div>;
  ...
  return (
    <div className="project-container">
      ...
      <button className="Add-btn">+New Developer</button>
      </div>
      ...
      <p className="project-status">
        <strong>Status:</strong> <span className="status-done">{project.status}</span>
      </p>

      <h2 className="developers-title">Developers</h2>
      <div className="developers-list">
        {project.listOfDevelopers.map((dev, index) => (
        <Link to={`/dev/${dev.devId}`}><div key={index} className="developer-card">
